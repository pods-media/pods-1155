...
const getFactoryImplConfigs = async () => {
  ...
  const chainConfigs = await Promise.all(
    addresseFiles.map(async (addressConfigFile) => {
      ...
      const chainConfig = JSON.parse(await readFile(
        path.resolve(__dirname, `../chainConfigs/${chainId}.json`), 'utf-8'
      ));
...
async function main() {
  ...
  const turnkeyAccount = privateKeyToAccount(
    process.env.SIGNER_PRIVATE_KEY! as Address,
